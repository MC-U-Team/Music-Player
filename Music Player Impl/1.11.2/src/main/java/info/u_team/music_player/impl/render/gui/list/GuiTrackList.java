...
public class GuiTrackList extends GuiScrollingList {
	...
	protected void drawSlot(int index, int entryRight, int slotTop, int slotBuffer, Tessellator tess) {
		...
		if (index == 0) {
			...
			gui.mc.fontRenderer.drawString(track == null ? I18n.format(MusicPlayerConstants.MODID + ":gui.queue.empty") : (track.getInfo().title.equals("Unknown title") ? track.getInfo().uri : track.getInfo().title), left + 5, slotTop, 0x00FF00);
		} else {
			track = trackscheduler.getQueue().get(index - 1);
			gui.mc.fontRenderer.drawString(track == null ? I18n.format(MusicPlayerConstants.MODID + ":gui.queue.null") : (track.getInfo().title.equals("Unknown title") ? track.getInfo().uri : track.getInfo().title), left + 5, slotTop, 0xFF0000);
		}
